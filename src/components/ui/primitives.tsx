...
export function PanelTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return (
    <h2 className={cn('text-[13px] font-semibold tracking-wide text-ink', className)} {...props} />
  )
}
...
const badgeVariants = cva(
  'inline-flex items-center gap-1.5 rounded border px-1.5 py-0.5 text-[11px] font-medium leading-tight',
  {
    variants: {
      tone: {
...
export function Mono({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) {
  return (
    <span
      className={cn('font-mono text-[11.5px] tracking-tight text-ink-subtle', className)}
      {...props}
    />
  )
}
  ...
  return (
    <div className={cn('flex items-end justify-between gap-6', className)}>
      <div className="min-w-0">
        ...
        {description && <p className="mt-1 max-w-3xl text-[13px] text-ink-muted">{description}</p>}
      </div>
      {action}
    </div>
  )
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-start gap-3 rounded-panel border border-dashed border-line-strong bg-surface-sunken/60 px-6 py-8">
      ...
      {children && (
        <div className="max-w-2xl text-[13px] leading-relaxed text-ink-muted">{children}</div>
      )}
      {action}
    </div>
  )
